...
namespace BK7231Flasher
{
    public class ESPFlasher : BaseFlasher
    {
        ...
        // ---- ESP32 auto-reset/download-mode helpers ----
        // Platform/driver-specific reset behaviour (Windows usbser.sys quirks, USB-Serial-JTAG reset, etc.)
        ...
        bool openPort()
        {
            try
            {
                if (serial == null)
                {
                    ...
                    // Bind reset helper to this port and force a known-neutral modem-control state.
                    _reset.Bind(serial);
                    _reset.SetNeutral();
                }
                else
                {
                    ...
                    // Re-bind reset helper in case the device was re-enumerated
                    _reset.Bind(serial);
                }
        ...
        public bool Connect()
        {
            ...
            if (!TryReset(() => _reset.ClassicReset()))
            {
            ...
            if (!Sync())
            {
                ...
                addLogLine("Resetting into download mode (USBJTAGSerialReset)...");
                if (!TryReset(() => _reset.USBJTAGSerialReset()))
                {
